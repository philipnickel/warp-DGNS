...
from warp._src.fem.field import FieldLike, SeedField
...
def extract_qfunction(
    ...
    Raises:
        NotImplementedError: If the quadrature domain is not a cell domain
            (DG face/side forms are not supported yet: the seeds cannot
            distinguish inner from outer traces, so e.g. ``jump(v)`` would
            silently extract as zero), or if the integrand accesses the test
            field through an operator other than ``inner``/``outer``/``grad``/
            ``grad_outer``/``degree`` (node-based operators are not
            representable as per-quadrature-point ``(f0, f1)`` coefficients).
    """

    domain = quadrature.domain
    if domain.element_kind != ElementKind.CELL:
        raise NotImplementedError(
            "Q-function extraction is only implemented for cell domains; side (DG face) forms are not supported yet"
        )
