...
| 3 · Fused apply + transparent dispatch | **DONE** (integration gate green) | `ec1491a9`; `warp/_src/fem/sumfac/kernels.py`, dispatch in `integrate.py`, `fem.set_sumfac_mode("auto"|"force"|"off")` / `WARP_FEM_SUMFAC`, `ValueInjectedField`; spike findings in `design/sumfac-phase3-spike-findings.md` |
| 4 · Bilinear assembly to BSR | **DONE** | `33e8c080`; channel-seeded action-on-unit-vectors, shared-memory fallback predicate |
| 5 · Face fallback + end-to-end DG | **DONE** | `43a048e2`; `test_fem_sumfac_faces.py` (dispatch-spy asserts sumfac volume + legacy sides, P=5 SIP solve force==off) |
...
- Sum-fac kernels have no autodiff (guarded: requires_grad inputs fall back to legacy).
