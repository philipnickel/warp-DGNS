...
| E_b · Wide-panel element batching (2D linear apply) | **DONE** (infrastructure; D stage now the recorded wall) | Explicit opt-in `integrate(..., assembly="sumfac", assembly_options={"element_batch": E_b})`: the fused 2D linear cell kernel runs every contraction stage as one wide GEMM over `E_b` elements (panels `(2q, n·E_b)`...), with only TWO per-element transpose restages — the Bᵀ stage consumes the TRANSPOSED coefficient blocks directly since `(opᵀ f op)ᵀ = opᵀ fᵀ op` has the same form, and the result lands in natural layout. Strided `tile_view`s feed only `tile_assign` (validated sound; `tile_matmul` remains the only corrupting consumer). 3D/bilinear/side and non-divisible element counts raise descriptive errors; cache key already discriminated on `element_batch`. Validated: `test_apply_equals_naive_element_batch` (mass + stiffness, P=4, E_b=2, CPU+CUDA) plus host-side `test_assembly_options_validation`; production results bit-identical across E_b∈{1,2,4,8} (rel err ≤5e-16). Design source: `design/spike_cutile_sumfac/` (NCU-driven; isolated-GEMM benefit −36% FP64 instructions, −25% ms). Production measurement (Grid2D 128² P=4 f64 stiffness, RTX 5090): kernel 864 µs at E_b=1 → E_b=4 saves the predicted ~25M GEMM DFMAs but total is ~490M FP64 inst, **~85–90% of which is the block-redundant seeded D stage** (240M DMULs: per-QP geometry decode + 3 seeded evals × 64 redundant lanes) → wall flat. CONCLUSION: the cell-apply wall is the D stage, not the GEMMs; E_b is the enabling infrastructure for when it shrinks. Next levers, evidence-ranked (spike: trivial-integrand scalar D costs 2.3× vs one `tile_map`): (1) constant-geometry / injected-mask D stage for affine grids (mirror of the side kernels' side-constant injection), (2) lane-distributed seeded evaluation, (3) `extract_qfunction` offline path + wide-GEMM consume. |
| QF · Extracted D stage (2D linear apply) | **DONE on CPU; CUDA validation pending a driver fix (see env notes)** | The D-stage redundancy wall (E_b row above) attacked per the curvilinear constraint: NO constant-geometry assumptions. `integrate(..., assembly="sumfac", assembly_options={"qfunction": "extracted"})` splits the linear cell apply into (1) a one-thread-per-quadrature-point extraction kernel (`make_qfunction_ref_eval_kernel_fn` in `qfunction.py`: per-point `w·|det J|` folding AND per-point `J^{-1}` reference-space mapping — curved/non-affine elements fully supported; the input field is NOT substituted, it evaluates natively per QP) and (2) an integrand-INDEPENDENT `B^T` consume kernel (`get_sumfac_qf_consume_kernel`, `cache.dynamic_kernel` keyed on shapes only — shared across integrands, fewer LTO compiles) using the wide-panel transposed block algebra; composes with `element_batch`. Rationale: the seeded D stage executes ~85–90% of the production kernel's FP64 work block-redundantly (each of 64 threads walks all QPs); extraction distributes QPs across threads (~64× less redundant lane work) at the cost of one fq array round trip ((E, 1+d, q²) ≈ 10 MB at 128², ~6 µs) and naive per-QP input evaluation in the extraction kernel (O(n²)/QP — comparable to the B stage in 2D P=4; a separate B-stage feeder kernel is the 3D/high-P follow-up). Validated vs the BSR oracle on CPU: mass + stiffness on Grid2D, stiffness on the vertex-perturbed NON-AFFINE Quadmesh2D (the curvilinear gate), and stiffness with `element_batch=2`; plus host-side option validation. CUDA validation + production bench BLOCKED mid-session by a driver/library mismatch (see env notes); run `uv run warp/tests/fem/test_fem_sumfac_apply.py` after the driver fix, then re-run the production bench (expect the D-stage wall to drop substantially). |
...
- ~~`E_b > 1` is implemented only in the standalone contraction primitives, not the fused kernels~~ **Resolved for the
  2D linear cell apply** (`assembly_options={"element_batch": E_b}`, see the E_b phase row); still open for 3D,
  bilinear, and side kernels. Production wall is the seeded D stage, so E_b is currently perf-neutral end-to-end.
...
- **2026-06-11 driver breakage (action needed)**: mid-session, the system's NVIDIA userland libraries were updated
  to 580.159.03 (likely unattended-upgrades) while the loaded kernel module remained 580.95.05 →
  `nvidia-smi` fails with "Driver/library version mismatch" and CUDA init returns error 999. The 580.159.03
  kernel module IS on disk (dkms); module refcounts showed no active GPU users. Fix: reload the modules
  (`rmmod nvidia_uvm nvidia_drm nvidia_modeset nvidia && modprobe nvidia_uvm`) or reboot. After the fix, finish
  the QF phase: `uv run warp/tests/fem/test_fem_sumfac_apply.py` (CUDA halves of the element_batch and
  qfunction-extracted oracle tests) and the production bench.
